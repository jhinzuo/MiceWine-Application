...
static int xcallback(int fd, int events, __unused void* data) {
    ...
    if (events & (ALOOPER_EVENT_ERROR | ALOOPER_EVENT_HANGUP)) {
        jobject instance = (*env)->CallStaticObjectMethod(env, EmulationActivity.self, EmulationActivity.getInstance);
        if (instance)
            (*env)->CallVoidMethod(env, instance, EmulationActivity.clientConnectedStateChanged);
